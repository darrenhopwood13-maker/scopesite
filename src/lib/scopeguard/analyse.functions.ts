  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      try {
        ...
        const { detectPartyDisclaimers } = await import("./pipeline");
        await refreshPartyRegister(
          supabase,
          drawing.id,
          { project_id: drawing.project_id, owner_id: drawing.owner_id },
          detectPartyDisclaimers(extract.items, extract.titleblock.originator),
        );
      } catch {
